...
import { CarSpottingFormData, carSpottingSchema } from '../schema'
...
export function SpottingForm() {
  ...
  const {
    ...
    reset,
  } = useForm<CarSpottingFormData>({
    resolver: zodResolver(carSpottingSchema),
  })
  ...
  async function onSubmit(data: CarSpottingFormData) {
    try {
      ...
      const response = await fetch('/api/car-spottings', {
        method: 'POST',
        headers: {
      ...
      if (!response.ok) {
        throw new Error('Erro ao salvar car spotting')
      }

      customRevalidateTag('api/car-spottings')
      toast.success('Car spotting guardado com sucesso!!')
      reset()
      setSelectedFile(null)
    } catch (error) {
      toast.error('Erro ao salvar car spotting')
    }
  }

  return (
    <div className="mx-auto max-w-3xl rounded-lg bg-white p-6 shadow-md">
      <h2 className="mb-4 text-2xl font-bold">Adicionar Novo Car Spotting</h2>
